...
class Scratch3DataBlocks {
    ...
    showVariable (args, util) {
        const value = this.getVariable(args, util);
        console.log(`${args.VARIABLE.name} = ${value}`);
    }
    ...
    showList (args, util) {
        const value = this.getListContents(args, util);
        console.log(`${args.LIST.name} = ${value}`);
    }
